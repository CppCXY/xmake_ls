...
pub fn infer_import_call(
    ...
    let import_path_type = infer_expr(db, cache, first_arg)?;
    let module_path: String = match &import_path_type {
    ...
    let module_info = db
        .get_module_index()
        .find_import(db, &module_path, cache.get_file_id())
